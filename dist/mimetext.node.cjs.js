...
class d {
    ...
    set(e, t) {
        ...
        if (!!this.fields.some(s)) {
            const n = this.fields.findIndex(s),
                i = this.fields[n];
            if (i.validate && !i.validate(t))
                throw new a(
                    "MIMETEXT_INVALID_HEADER_VALUE",
                    `The value for the header "${e}" is invalid.`,
                );
    ...
    validateMailboxMulti(e) {
        return e instanceof o || this.isArrayOfMailboxes(e);
    }
    dumpMailboxMulti(e) {
        const t = (e) =>
            0 === e.name.length
                ? e.dump()
                : `=?utf-8?B?${this.envctx.toBase64(e.name)}?= <${e.addr}>`;
        return this.isArrayOfMailboxes(e)
            ? e.map(t).join(`,${this.envctx.eol} `)
            : e instanceof o
              ? t(e)
              : "";
    }
    ...
    isArrayOfMailboxes(e) {
        return this.isArray(e) && e.every((e) => e instanceof o);
    }
    isArray(e) {
...
class u {
    constructor(e, t, s = {}) {
        (this.envctx = e),
            (this.headers = new h(this.envctx)),
            (this.data = t),
            this.setHeaders(s);
    }
    dump() {
...
class l {
    ...
    constructor(e) {
        (this.envctx = e),
            (this.headers = new d(this.envctx)),
            (this.messages = []),
    ...
    addAttachment(e) {
        ...
        if (!1 === this.envctx.validateContentType(t))
            throw new a(
                "MIMETEXT_INVALID_MESSAGE_TYPE",
                `You specified an invalid content type "${t}".`,
            );
        const s =
            e.headers["Content-Transfer-Encoding"] ?? e.encoding ?? "base64";
        this.validContentTransferEncodings.includes(s) ||
            (t = "application/octet-stream");
        const n = e.headers["Content-ID"];
        "string" == typeof n &&
            n.length > 2 &&
            !n.startsWith("<") &&
            !n.endsWith(">") &&
            (e.headers["Content-ID"] = "<" + e.headers["Content-ID"] + ">");
        const i = e.inline ? "inline" : "attachment";
        return (
            (e.headers = Object.assign({}, e.headers, {
                "Content-Type": `${t}; name="${e.filename}"`,
                "Content-Transfer-Encoding": s,
                "Content-Disposition": `${i}; filename="${e.filename}"`,
            })),
    ...
    addMessage(e) {
        ...
        if (!this.validTypes.includes(t))
            throw new a(
                "MIMETEXT_INVALID_MESSAGE_TYPE",
                `Valid content types are ${this.validTypes.join(", ")} but you specified "${t}".`,
            );
        const s =
            e.headers["Content-Transfer-Encoding"] ?? e.encoding ?? "7bit";
        this.validContentTransferEncodings.includes(s) ||
            (t = "application/octet-stream");
        const n = e.charset ?? "UTF-8";
        return (
            (e.headers = Object.assign({}, e.headers, {
                "Content-Type": `${t}; charset=${n}`,
                "Content-Transfer-Encoding": s,
            })),
            this._addMessage({ data: e.data, headers: e.headers })
        );
    ...
    _addMessage(e) {
        const t = new u(this.envctx, e.data, e.headers);
    ...
    setRecipients(e, t = { type: "To" }) {
        const s = (this.isArray(e) ? e : [e]).map((e) => new o(e, t));
...
const c = {
    ...
    eol: e.EOL,
    validateContentType: (e) => r.contentType(e),
};
(exports.MIMEMessage = l),
    (exports.MIMEMessageContent = u),
    (exports.MIMEMessageHeader = d),
    (exports.MIMETextError = a),
    (exports.Mailbox = o),
